...
def capture():
    print ('capturing')
    sock.sendto("shoot", (MCAST_GRP, MCAST_PORT)) #sending command shoot
    ...
    def ReadfromDSLR(): 
        ...
        def ReadConfig():
            ...
            if iFile.isalpha():
                if iFile =='ALL':
                    ...
                    iFile = "Configuration.txt"
                    RF=1
                else:
                    print("Not an option")
            elif int(iFile) in range(1,60):
                
                iFile = "Configuration"+iFile+".txt"
                RF=1
        ...
        def update():  # this function is defind inside the configure() to ease the declaration of variables (global stuff )
            ...
            if camera.isalpha():
                if camera == 'ALL':
                    for i in range(1,60):
                        outfile="Configuration"+str(i)+".txt"
                        writing(outfile)
                        flag=1
                else:
                ...
                outfile="Configuration"+camera+".txt"
                writing(outfile)
                flag=1
            ...
            if flag == 1:
                ...
                sock.sendto("configure", (MCAST_GRP, MCAST_PORT)) #Sending commad configure, Each Rpi will open the configure.py to configure the Cams
...
def Reboot():
    print "Reboot all"
    sock.sendto("reboot", (MCAST_GRP, MCAST_PORT)) #sending command Reboot    

def USBreset():
    print "USB reset to all"
    sock.sendto("resetusb", (MCAST_GRP, MCAST_PORT)) #sending command USBReset
